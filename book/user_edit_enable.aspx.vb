...
''' 作者：陈强
''' 网页：修改无效成员页面
''' 内容：主要是完成对无效成员的员工号、用户名以及使其生效的操作
''' </summary>
''' <remarks></remarks>
Partial Class user_edit_enable
    ...
    Protected Sub loseEff_Click(sender As Object, e As EventArgs) Handles loseEff.Click
        ...
        Try
            ...
            myconn = bCommon.GetConnection()
            '拼写sql语句
            Dim sqlStr As String = "UPDATE USER_INFO SET VALID_FLAG = '0' WHERE USER_ID =" + Session("USER_ID")
            myconn.Open()
            dbCommand = New SqlCommand(sqlStr, myconn)
            dbCommand.ExecuteNonQuery()
            dbCommand.Dispose()
    ...
    ''' <remarks></remarks>
    Protected Sub setData2(ByVal USER_ID As String)
